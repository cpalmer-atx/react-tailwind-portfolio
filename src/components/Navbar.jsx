...
const Navbar = () => {
  ...
  return (
    <nav className="relative container mx-auto p-6  max-w-5xl bg-gradient-to-tr from-primary via-primary to-secondary">
      <div className="flex items-center justify-between">
        ...
        { nav && (
          <ul className="flex flex-col md:hidden z-10 justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-tr from-primary via-primary to-secondary text-primaryTxt">
            <h1 className="text-4xl py-3 my-4 px-10 border-b-2 border-primaryTxt font-logo text-transparent bg-clip-text bg-gradient-to-bl from-primaryTxt via-primaryTxt to-secondaryTxt">C.Palmer</h1>
            {links.map( (link) => (
              <li key={link} onClick={() => setNav(!nav)} className="cursor-pointer py-4 text-2xl hover:text-secondaryTxt hover:scale-110">
                <Link to={link} smooth duration={500}>{link}</Link>
              </li>
            ))}
